...
bool DJSession::load_track_to_mixer_deck(const std::string& track_title) {
    ...
    int place=mixing_service.loadTrackToDeck(*track);
    if(place==0){
        stats.deck_loads_a++;
        stats.transitions++;
    }
    ...
    if(place==-1){
        std::cerr << "[ERROR] Mixer failed to load track: \"" << track_title << "\"\n";
        stats.errors++;
        return false;
    }
    return true;
...
void DJSession::simulate_dj_performance() {
    ...
    if (play_all) {
        for (const auto& p : session_config.playlists) {
            playlists_to_process.push_back(p.first); 
        }
    ...
    //(c)-1-2
    for (const std::string& playlist_name : playlists_to_process) {
    ...
    //(c)-3
      ...
      for(const std::string& track_title : track_titles) {
        ...
        //(c) 3.4
        bool ok = load_track_to_mixer_deck(track_title);
