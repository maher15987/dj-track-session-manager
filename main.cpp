...
bool LRUCache::put(PointerWrapper<AudioTrack> track) {
    ...
    size_t index = findSlot(trackName);
    if (index != max_size) {
    ...
    size_t emptyIdx = findEmptySlot();
    if (emptyIdx == max_size) {
        return evicted;
    }

    slots[emptyIdx].store(std::move(track), access_counter);
    return evicted;
...
size_t LRUCache::findLRUSlot() const {
    ...
    for (size_t i = 0; i < max_size; ++i) {
        if (slots[i].isOccupied()) {
            if (slots[i].getLastAccessTime() < lru_time) {
